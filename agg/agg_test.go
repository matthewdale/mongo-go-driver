...
func Example() {
	_ = agg.Abs(agg.BitOr(agg.NumberField("myField")))
	_ = agg.Abs(agg.BitOr(agg.Number(5)))

	pipeline := agg.Pipeline{
		agg.SortStage(
			agg.SortBy("score", agg.SortTextScore),
			agg.SortBy("blah", agg.SortDescending),
		),
	}
...
func Test_computeAggregates(t *testing.T) {
	...
	pipeline := agg.Pipeline{
		agg.GroupStage(
			...
			agg.AccField(
				"numChanges",
				agg.AvgAcc(agg.Size(agg.ArrayField("$prices"))),
			),
			agg.AccField(
				"lowestPrice",
				agg.MinAcc(agg.NumberField("$prices.price")),
			),
			agg.AccField(
				"highestPrice",
				agg.MaxAcc(agg.NumberField("$prices.price")),
			),
			agg.AccField(
				"weightedAveragePrice",
				agg.AvgAcc(agg.NumberField("$weightedAveragePrice")),
			),
			agg.AccField(
				"percentiles",
				agg.PercentileAcc(
		...
		agg.SetStage(agg.Field(
			"percentiles",
			agg.ArrayToObject(agg.Zip(
				[]agg.ResolvesToArray{
					agg.Array(percentiles),
